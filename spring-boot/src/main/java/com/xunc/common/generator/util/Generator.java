...
public class Generator {
     ...
     * 模块名
     */
    private final static String MODEL_NAME = "admin";
    /**
     * 表名 (可以同时操作多个, 建议同时操作同一模块下的表)
     */
    private final static String[] TABLE_NAME = new String[]{"ad_user"};
    /**
     * 主键生成策略
     */
    ...
    public static void dataBaseConfig() throws IOException {
        ...
        Map shopjy = (Map) datasource.get("druid");
        // 数据库链接
        dataSourceConfig.setUrl((String) shopjy.get("url"))
                ...
                // 配置数据库连接用户名
                .setUsername((String) shopjy.get("username"))

                // 配置数据库连接密码
                .setPassword(shopjy.get("password").toString());
